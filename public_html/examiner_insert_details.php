...
<body>
	<script>
		...
		if(go=="h"){
				async function foo(){
					var d1="<?php echo $file['name'];?>";
					console.log(d1);
					const response = await fetch("upload-file/"+d1);
					...
					console.log(dataget.report.appProperties);
						...
						log.id="im";
						var gh = "<?php echo $image['name'];?>";
						console.log(gh);
						log.src="upload-images/"+gh;
						...
						<tr><td>Examiner</td>
						<td><?php $examiner;?></td></tr>
						<tr><td>Case Folder</td>
						<td><?php $casefolder;?></td></tr>
						<tr><td>F.I.R</td>
						<td><?php $fir?></td></tr>
						<tr><td>Destination</td>
						<td><?php $destination;?></td></tr>
						<tr><td>Date Time</td>
						<td><?php $date;?></td></tr></table>`;
